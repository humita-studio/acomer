...
export function ResumenPago({ ticket }: ResumenPagoProps) {
  ...
  return (
    <div className="min-h-screen bg-muted p-4 sm:p-8 flex flex-col items-center justify-start pb-24">
      <div className="w-full max-w-md bg-card rounded-3xl shadow-lg border border-border overflow-hidden animate-in slide-in-from-bottom-10 fade-in">
        
        {/* Header (Ticket Top) */}
        <div className={`p-8 text-center text-white ${isApproved ? 'bg-success' : isPending ? 'bg-warning' : isCanceled ? 'bg-destructive' : 'bg-foreground'}`}>
          <div className="w-16 h-16 bg-card/20 backdrop-blur-sm rounded-full flex items-center justify-center mx-auto mb-4">
            {isApproved ? (
        ...
        <div className="p-6 sm:p-8 space-y-6">
          <div className="flex justify-between items-center text-sm border-b pb-4">
            ...
            <span className="font-semibold text-foreground">{getMetodoLabel(ticket.transaccion.proveedor)}</span>
          </div>

          <div className="space-y-4">
            <h3 className="font-bold text-foreground text-sm uppercase tracking-wider">Tu Pedido</h3>
                ...
                <div key={idx} className="flex justify-between items-start text-sm">
                  <div className="flex-1">
                    ...
                    {item.modificadores.length > 0 && (
                      <div className="text-muted-foreground text-xs mt-0.5 ml-4">
          ...
          <div className="border-t border-dashed border-border pt-4 mt-6">
            {ticket.totalPagado && ticket.totalPagado > 0 ? (
              <div className="space-y-2">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-semibold text-foreground">${(ticket.saldoPendiente || 0) + ticket.totalPagado}</span>
                </div>
                <div className="flex justify-between items-center text-sm text-success-foreground">
                  <span>Pagos previos aprobados</span>
                  <span>- ${ticket.totalPagado.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center pt-2">
                  <span className="font-bold text-muted-foreground text-lg">RESTANTE</span>
            ...
            {isPending && ticket.transaccion.proveedor !== 'mercado_pago' && (
              <p className="text-center text-warning-foreground bg-warning-subtle p-3 rounded-lg text-sm font-medium mt-6">
                Un mozo se está acercando a tu mesa para realizar el cobro.
              </p>
            )}
            {isCanceled && (
              <p className="text-center text-destructive bg-destructive-subtle p-3 rounded-lg text-sm font-medium mt-6">
        ...
        <div className="p-6 bg-muted border-t">
          <button
            ...
            className="w-full bg-card border-2 border-border text-foreground font-bold py-4 rounded-xl hover:bg-muted hover:border-border transition-all shadow-sm"
          >
            Volver a la Carta
          </button>
        </div>
