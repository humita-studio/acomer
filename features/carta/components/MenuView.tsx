...
export function MenuView({
  ...
  const visibleProducts = useMemo(() => {
    if (!buscando) {
      return productos.filter((p) => p.categoriaId === activeCategory);
    }
  ...
  }, [productos, activeCategory, buscando, query]);
  ...
  return (
    <div className="pb-24 max-w-2xl mx-auto w-full relative min-h-screen bg-muted/30">
      {/* Sólo tabs/búsqueda/acciones van sticky. La lista de productos queda
          afuera: con cientos de ítems el sticky dejaría de fijarse. */}
              ...
              <div className="min-w-0">
                {buscando && (
                  <p className="mb-0.5 text-xs font-medium text-muted-foreground">
                    {catNombre.get(prod.categoriaId)}
                  </p>
                )}
